...
const Index = () => {
  ...
  const hiringCount = jobs.filter(j => j.post_type === 'hiring').length;
  const seekingCount = jobs.filter(j => j.post_type === 'seeking').length;

  const stats = [
    { icon: <Briefcase className="h-6 w-6" />, value: totalJobs, label: t('stats.jobs', language) },
    { icon: <Users className="h-6 w-6" />, value: hiringCount, label: t('stats.employers', language) },
    { icon: <TrendingUp className="h-6 w-6" />, value: seekingCount, label: t('stats.seekers', language) },
  ];
            ...
            {categories.map((cat) => (
              <Link key={cat.id} to={`/jobs?category=${cat.id}`}>
                <motion.div whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}
                  ...
                  <span className="text-xs font-medium whitespace-nowrap">{t(`cat.${cat.id}` as any, language)}</span>
                </motion.div>
              </Link>
            ))}
          </div>
        ...
        <div className="container">
          ...
          {totalJobs > 6 && (
            <div className="text-center mt-6">
              <Link to="/jobs"><Button variant="outline">{t('nav.findJobs', language)} →</Button></Link>
            </div>
          )}
        </div>
      </section>
